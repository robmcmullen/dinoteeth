...
#    sticky_parser.add_argument("-n", action="store", dest="name", default='', help="Movie or TV Series name")
    ...
    sticky_parser.add_argument("--no-dvdnav", action="store_false", dest="dvdnav", default=True, help="Use libdvdnav (default True)")
    ...
    sticky_parser.add_argument("--hb-normalize", action="store_true", default=False, help="Experimental handbrake normalization")
    ...
    title_parser.add_argument("--no-burn", action="store_true", default=False, help="Don't burn any subtitles, even if forced subtitles would have been found")
        ...
        for options in title_options:
            ...
            if options.episode is not None:
                ...
                for dvd_title, episode, name in zip(dvd_titles, numbers, names):
                    ...
                    try:
                        encoder = HandBrakeEncoder(source, scan, filename, dvd_title, audio, subtitles, options, bonus=True)
                        queue.append(encoder)
                    except HandBrakeScanError:
                        print "Bad title number %s!  Skipping." % dvd_title
