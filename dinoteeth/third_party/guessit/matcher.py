...
from guessit.patterns import video_exts, subtitle_exts, sep, deleted, video_rexps, websites, episode_rexps, weak_episode_rexps, non_episode_title, find_properties, canonical_form, unlikely_series, bonus_rexps
...
#        language, span, confidence = search_language(current)
#
#    log.debug("9-->%s<--" % current)
    
    for rexp, confidence, span_adjust in bonus_rexps:
        match = re.search(rexp, current, re.IGNORECASE)
        if match:
            metadata = match.groupdict()
            guess = guessed(metadata, confidence = confidence)
            current = update_found(current, guess, match.span(), span_adjust)
    log.debug("10-->%s<--" % current)
    ...
    # split into '-' separated subgroups (with required separator chars around
    # the dash).  Note: finds only single dashes.  Multiple consecutive dashes
    # are left in the text as a user-intended separator
    dash_rexp = re.compile("(?<!-)-(?!-+)")
    match = dash_rexp.search(current)
    while match:
        didx = match.start(0)
        regions.append(((didx, didx), None))
        match = dash_rexp.search(current, match.end(0))
...
def match_from_epnum_position(match_tree, epnum_pos, guessed, update_found):
    ...
    log.debug("series_candidates: %s" % str(series_candidates))
    ...
    def __init__(self, filename, filetype = 'autodetect'):
        ...
        def update_found(leftover, group_pos, guess):
            ...
            match_tree[pidx][eidx][gidx] = (group[0],
                                            deleted * len(group[0]),
                                            guess)
            log.debug("--update_found-->%s<--" % str([ g for g in leftover if g[1] != group_pos ]))
        ...
        log.debug("match_tree-->%s" % match_tree)
        ...
        if filetype in ('episode', 'episodesubtitle'):
            ...
            if eps:
                ...
                # Also check the extra feature number
                eps = find_group(match_tree, 'extraNumber')
                if eps:
                    match_tree = match_from_epnum_position(match_tree, eps[0], guessed, update_found)
            ...
            log.debug("--leftover-->%s<--" % str(leftover))
            ...
            eps = find_group(match_tree, 'extraNumber')
            if eps:
                log.debug("--movie-->%s<--" % "found extraNumber")
                log.debug("--leftover-->%s<--" % str(leftover_all))
                log.debug("--eps-->%s<--" % str(eps))
                log.debug("--match_tree-->%s<--" % str(match_tree))
                def same_egroup_after(group):
                    _, (ppidx, eeidx, ggidx) = group
                    log.debug("---->%s, p=%d e=%d g=%d" % (_, ppidx, eeidx, ggidx))
                    return ppidx == eps[0][0] and eeidx == eps[0][1] and ggidx > eps[0][2]
                title_candidates = filter(same_egroup_after, leftover_all)
                log.debug("--title_candidates-->%s" % str(title_candidates))
                if title_candidates:
                    guess = guessed({ 'extraTitle': title_candidates[0][0] }, confidence = 0.4)
                    leftover = update_found(leftover_all, title_candidates[0][1], guess)
