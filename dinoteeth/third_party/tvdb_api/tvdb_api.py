...
class Tvdb:
    ...
    def __init__(self,
        ...
        self.config['url_getImdb'] = u"%(base_url)s/api/GetSeriesByRemoteID.php?imdbid=%%s" % self.config
    ...
    def _getShowByImdb(self, imdb_id, language):
        """Takes an IMDb ID and returns a Show
        """
        summary = self._getetsrc(self.config['url_getImdb'] % (imdb_id))
        series = summary.find('Series')
        if series:
            sid = int(series.find('id').text)
            if sid not in self.shows:
                self._getShowData(sid, self.config['language'])
            self.shows[imdb_id] = self.shows[sid]
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, (int, long)):
            ...
            if key not in self.shows:
                self._getShowByImdb(key, self.config['language'])
